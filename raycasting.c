...
/*   Updated: 2020/10/02 13:35:05 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_DDA(t_eng *eng, t_map *map)
{
	while (1)
	{
		...
		if (map->map[eng->Dpos[0]][eng->Dpos[1]] > 0)
			break;
	}
}

static void	ft_draw_screen(t_eng *eng, t_mega *mega, int Wmin, int Wmax, int x)
{
	int	y;
	...
	while (++y < mega->map.res[1])
		mlx_pixel_put(mega->mlxp, mega->win, x, y, eng->side == 1 ? ft_get_color(mega->map.ceiling) : (ft_get_color(mega->map.ceiling) / 2));
}
...
static void	ft_draw_wall(t_eng *eng, t_map *map, t_mega *mega, int x)
{
	...
	ft_draw_screen(eng, mega, Wmin, Wmax, x);
}
